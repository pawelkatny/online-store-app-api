...
const getSummary = async (req, res) => {
  ...
  res.status(StatusCodes.OK).json(success(StatusCodes.OK, { summary }));
};
...
const updateInfo = async (req, res) => {
  ...
  const user = await CustomerService.updateInfo(currentUser.id, {
    name,
    email,
    phone,
  });
...
const getCart = async (req, res) => {
  ...
  res.status(StatusCodes.OK).json(success(StatusCodes.OK, { cart }));
};
...
const updateProductCart = async (req, res) => {
  ...
  const { qty } = req.body;
  const cart = await CustomerService.updateProductCartQty(
    currentUser.id,
    productId,
    qty
  );
...
const removeFromCart = async (req, res) => {
  const currentUser = req.user;
  const { productId } = req.params;
  const cart = await CustomerService.removeProductFromCart(
    currentUser.id,
    productId
  );
  if (!cart) {
...
const addProductToFav = async (req, res) => {
  const currentUser = req.user;
  const { productId } = req.body;
  const favProduct = await CustomerService.addProductToFav(
    currentUser.id,
    productId
  );
  if (!favProduct) {
...
const removeProductFromFav = async (req, res) => {
  const currentUser = req.user;
  const { productId } = req.params;
  const favProduct = await CustomerService.removeProductFromFav(
    currentUser.id,
    productId
  );
  if (!favProduct) {
...
const checkout = async (req, res) => {
  ...
  res.status(StatusCodes.OK).json(success(StatusCodes.OK, { checkout }));
};
...
const showReturn = async (req, res) => {
  ...
  const returnDoc = await ReturnService.getCustomerReturn(
    currentUser.id,
    returnId
  );
